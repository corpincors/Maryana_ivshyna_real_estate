...
import { X, Home, Layers, Camera, Plus, Phone } from '../../components/Icons';
...
const PropertyFormModal: React.FC<PropertyFormModalProps> = ({ 
  ...
  useEffect(() => {
    if (editingProperty) {
      ...
      availableTechOptions, availableComfortOptions, availableCommOptions, availableInfraOptions
  ]);
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    ...
    setFormData((prev: Partial<Property>) => ({ ...prev, [name]: val }));
  };

  const handleCategoryChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newCategory = e.target.value;
    setFormData((prev: Partial<Property>) => ({ 
      ...prev, 
  ...
  const handleDistrictChange = (value: string) => {
    setFormData((prev: Partial<Property>) => ({ ...prev, district: value }));
  };

  const handleYearBuiltChange = (value: string) => {
    setFormData((prev: Partial<Property>) => ({ ...prev, yearBuilt: value }));
  };

  const handleWallTypeChange = (value: string) => {
    setFormData((prev: Partial<Property>) => ({ ...prev, wallType: value }));
  };

  const handleBathroomTypeChange = (value: string) => {
    setFormData((prev: Partial<Property>) => ({ ...prev, bathroomType: value }));
  };

  const handleHousingClassChange = (value: string) => {
    setFormData((prev: Partial<Property>) => ({ ...prev, housingClass: value }));
  };

  const handleRepairTypeChange = (value: string) => {
    setFormData((prev: Partial<Property>) => ({ ...prev, repairType: value }));
  };

  const handleHeatingChange = (value: string) => {
    setFormData((prev: Partial<Property>) => ({ ...prev, heating: value }));
  };
  ...
  const removePhoto = (index: number) => {
    setFormData((prev: Partial<Property>) => ({ ...prev, imageUrls: (prev.imageUrls || []).filter((_: string, i: number) => i !== index) }));
  };

  const handleToggle = (name: keyof Property) => {
    setFormData((prev: Partial<Property>) => ({ ...prev, [name]: !prev[name] }));
  };
