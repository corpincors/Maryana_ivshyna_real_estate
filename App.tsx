...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (params.get('clientMode') === 'true') {
      setIsClientMode(true);
    }
  }, []);

  // Compute the list of unique districts from defaults and current properties
      ...
      <section className="bg-white p-8 lg:p-12 rounded-[3.5rem] shadow-sm border border-slate-50 mb-12">
        <div className="grid grid-cols-1 gap-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-10">
            {/* DISTRICTS FILTER - Now using dynamically computed list */}
